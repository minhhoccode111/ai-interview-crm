...
def test_language_config():
    ...
    assert vi_config['tfidf_stopwords'] is None, "Vietnamese should have no built-in stopwords"
    ...
    assert en_config['tfidf_stopwords'] == 'english', "English should use built-in stopwords"
...
def test_vietnamese_questions():
    ...
    has_vietnamese = any(any(char in question for char in vietnamese_chars) for question in vietnamese_questions)
...
def test_whisper_language_codes():
    ...
    whisper_supported = [
        'en', 'vi', 'zh', 'ja', 'ko', 'fr', 'de', 'es', 'it', 'pt', 'ru', 'ar', 'hi'
    ]
