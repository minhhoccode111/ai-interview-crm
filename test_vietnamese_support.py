...
def test_ai_engine_vietnamese():
    ...
    sample_resume = {
        ...
        "projects": [{"name": "Web Application", "description": "E-commerce platform"}],
    }
    ...
    try:
        ...
        vietnamese_found = any(
            "bạn" in q.lower() or "của" in q.lower() or "về" in q.lower()
            for q in questions
        )
        if vietnamese_found:
...
def test_voice_processor_vietnamese():
    ...
    try:
        ...
        if "not found" in str(e).lower() or "no such file" in str(e).lower():
            print(
                "✅ Method signature test passed - Vietnamese language parameter accepted"
            )
        else:
...
def test_fallback_questions():
    ...
    try:
        ...
        if questions and any(
            "bạn" in q.lower() or "của" in q.lower() for q in questions
        ):
