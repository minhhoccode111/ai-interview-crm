...
class InterviewEngine:
    ...
    def generate_questions(self, resume_data):
        ...
        prompt = f"""
        Based on this candidate's profile, generate exactly 10 interview questions.
        ...
        Return each question on a new line, numbered 1-10.
        """
    ...
    def evaluate_answer(self, question, answer):
        ...
        if not answer.strip():
            return {
                ...
                'suggestions': ["Take time to think about your response before answering."]
            }
        ...
        try:
            ...
            return {
                ...
                'suggestions': ["Use the STAR method for behavioral questions", "Include concrete examples"],
    ...
    def _parse_feedback(self, feedback_text):
        ...
        # Extract strengths
        strengths_match = re.search(r'STRENGTHS:\s*(.+?)(?=IMPROVEMENTS:|$)', feedback_text, re.IGNORECASE | re.DOTALL)
        if strengths_match:
            strengths_text = strengths_match.group(1).strip()
            result['strengths'] = [s.strip() for s in re.split(r'[•\-*\n]', strengths_text) if s.strip()]
        
        # Extract improvements
        improvements_match = re.search(r'IMPROVEMENTS:\s*(.+?)(?=SUGGESTIONS:|$)', feedback_text, re.IGNORECASE | re.DOTALL)
        if improvements_match:
            improvements_text = improvements_match.group(1).strip()
            result['improvements'] = [s.strip() for s in re.split(r'[•\-*\n]', improvements_text) if s.strip()]
        
        # Extract suggestions
        suggestions_match = re.search(r'SUGGESTIONS:\s*(.+?)$', feedback_text, re.IGNORECASE | re.DOTALL)
        if suggestions_match:
            suggestions_text = suggestions_match.group(1).strip()
            result['suggestions'] = [s.strip() for s in re.split(r'[•\-*\n]', suggestions_text) if s.strip()]
    ...
    def generate_overall_evaluation(self, transcript, evaluations):
        ...
        avg_score = np.mean([eval_data.get('score', 0) for eval_data in evaluations]) if evaluations else 0
    ...
    def _parse_overall_evaluation(self, evaluation_text):
        ...
        for field in ['overall_score', 'technical_skills', 'communication', 'problem_solving']:
        ...
        # Extract text fields
        for field in ['summary', 'strengths', 'areas_for_improvement', 'recommendations']:
