...
def upload_resume(current_user):
    ...
    try:
        ...
        if 'file' in request.files:
            ...
            filename = f"resume_{current_user.id}_{uuid.uuid4().hex}_{original_filename}"
            ...
            return jsonify({'error': 'No resume data provided. Please upload a PDF file or provide text.'}), 400
        ...
        try:
            ...
            return jsonify({'error': 'Failed to parse resume. Please check the content and try again.'}), 500
...
def start_interview(current_user):
    ...
    try:
        ...
        language = data.get('language', current_user.preferred_language or Config.DEFAULT_LANGUAGE)
        ...
        if not resume.parsed_data:
            return jsonify({'error': 'Resume data is incomplete. Please upload the resume again.'}), 400
    ...
    try:
        ...
        if audio_file and audio_file.filename:
            ...
            try:
                ...
                answer_text = voice_processor.speech_to_text(filepath, language=interview_language)
                if not answer_text or answer_text.startswith("Error") or answer_text.startswith("Audio"):
                    return jsonify({'error': 'Could not transcribe audio. Please try again or use text input.'}), 400
            except Exception as e:
                print(f"Audio processing error: {e}")
                return jsonify({'error': 'Failed to process audio. Please try text input.'}), 500
            finally:
                # Clean up audio file
            ...
            return jsonify({'error': 'Either audio file or text answer is required'}), 400
...
def complete_interview(current_user, interview_id):
    ...
    try:
        ...
        evaluations = interview.evaluation.get('answers', []) if interview.evaluation else []
        ...
        try:
            overall_eval = engine.generate_overall_evaluation(interview.transcript, evaluations)
        except Exception as e:
            print(f"Overall evaluation error: {e}")
            # Fallback evaluation
            scores = [eval_data.get('evaluation', {}).get('score', 0) for eval_data in evaluations]
        ...
        report_data = {
            ...
            'duration': str(interview.end_time - interview.start_time) if interview.start_time else 'Unknown',
        ...
        return jsonify({
            ...
            'overall_score': overall_eval.get('overall_score', 0),
            'report_url': f"/static/uploads/{report_filename}" if interview.report_path else None,
            'summary': overall_eval.get('summary', ''),
            'strengths': overall_eval.get('strengths', []),
            'areas_for_improvement': overall_eval.get('areas_for_improvement', []),
            'recommendations': overall_eval.get('recommendations', []),
        ...
        return jsonify({'error': 'Failed to complete interview. Please try again.'}), 500
    ...
    try:
        ...
        for interview in interviews:
            history.append({
                ...
                'overall_score': interview.evaluation.get('overall_score', 0) if interview.evaluation else 0,
...
def get_interview_report(current_user, interview_id):
    ...
    try:
        ...
        report_data = {
            ...
            'detailed_analysis': {
                'summary': evaluation_data.get('summary', 'Interview completed successfully.'),
            ...
            'statistics': {
                'highest_score': max([ans['score'] for ans in detailed_answers], default=0),
                'lowest_score': min([ans['score'] for ans in detailed_answers], default=0),
                'questions_above_70': len([ans for ans in detailed_answers if ans['score'] >= 70]),
                'questions_below_50': len([ans for ans in detailed_answers if ans['score'] < 50])
            }
        }
